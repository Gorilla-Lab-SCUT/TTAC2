...
if os.path.exists('learned_prototype_cifar10.pth') is False or True:
    find_prototypes = FindPrototypes(classifier.fc, n_sample=1)
    # num_step=1000
    prototypes, prototypes_diverse = find_prototypes.inference()
    del find_prototypes
    torch.save({'learned_prototype': prototypes, 'prototypes_diverse': prototypes_diverse}, 'learned_prototype_cifar10.pth')
else:
    tmp = torch.load('learned_prototype_cifar10.pth')
    prototypes, prototypes_diverse = tmp['learned_prototype'].detach(), tmp['prototypes_diverse'].detach()

with torch.no_grad():
    bias = prototypes.max().item() / 30.
    # components
        ...
        for batch_idx, (inputs, _) in enumerate(tr_dataloader):
            ...
            if args.with_ssl:
                ...
                if args.ssl_sample == 'weak+strong':                    
                    images = torch.cat([inputs_0, inputs[1]], dim=0)
                    ...
                    inputs_0 = inputs_0.cuda(non_blocking=True)
            ...
            if iter_id > 0 and args.align_ext:
                if args.align_sample == 'none' and not args.with_ssl:
                ...
                if args.align_ext:
                    if not args.without_mixture:
                        ...
                        if args.filter != 'posterior':
                            ...
                            if (class_num == 10 or len(mini_batch_indices) >= 4096) and (iter_id > int(args.iters / 2) or args.filter == 'none'):
                                target_compoent_distribution.loc = new_component_mean / torch.norm(new_component_mean, dim=1, keepdim=True).detach() * ext_src_mu_scale
                    ...
                    if not args.without_global:
                        ...
                        target_domain = torch.distributions.MultivariateNormal(tmp_mu_weak * scale.item(), cov_src_ext)
