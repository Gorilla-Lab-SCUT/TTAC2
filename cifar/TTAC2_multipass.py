...
parser.add_argument('--with_constrastive', action='store_true', default=False)
        ...
        if is_both_activated and args.align_ext:
            ...
            if args.filter == 'ours':
                ...
                with torch.no_grad():
                    ...
                    posterior_tmp = posterior.max(dim=1, keepdim=True)[0] - math.log((2 ** 127) / 10)
                    posterior -= posterior_tmp
                    posterior = posterior.exp()
                    posterior /= posterior.sum(dim=1, keepdim=True)
                    posterior = posterior.transpose(0, 1).detach()
            else:
        ...
        if is_both_activated and type(loss) != float:
            ...
            if args.with_ssl:
                if args.ssl_sample == 'weak':
                    ...
                    del weak_logits, strong_logits, logits, weak_feats, strong_feats, feats, images, pro, target, mask, loss
            torch.cuda.empty_cache()
